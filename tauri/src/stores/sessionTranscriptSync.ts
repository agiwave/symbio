 ...
 * | — | — | — | 状态帧（`llm_state_frame` 剥掉了正文）：本地已有 ⇒ **零回读**只迁移状态；身份未知才回读补基线 |
 *
 ...
 * - **状态帧 + 身份未知**：`llm_state_frame` 剥掉了正文，本地没有该节点 ⇒ 只有回读
 *   才能拿到正文基线。
 *
 * 一旦回读在飞，`deltaGen` 就是它的作废基准：读取窗口内本端正文又变过（增量
 * 落地 / 全量替换落地）⇒ **丢弃回读的 `content`、只取身份**——本地正文**总是**
 * 真值的前缀，而回读快照可能落后。这条规则记的是「读取前该路径已应用过几次追加，
 * 响应回来若这个数变了就丢弃该响应」——它与 `useVdfs` 里详情视图的那一份是
 * **两个实例**：两个作废时机不同，共用一个会互相误伤。
 *
 ...
 * 增量，否则就是重复追加。
 *
 * 整份重读是另一条路：收到重同步信号（`event_bus` 的 `resync`，触发点有两个——
 * 后端通道曾满、本端重连成功）⇒ 按见过的会话 `reload`，权威快照直接替换本地转写。
 *
 * ## 合帧：为什么增量要攒一小会儿再落地
 *
 * 后端把窗口内的同节点增量合成一帧后才下发，但一帧仍不等于一次渲染：本层再按
 * 48ms 的窗口把**增量帧按会话攒批**一次交给落地口，避免每帧都付一次 store 提交
 * 与一次消息树重建（O(历史条数)）。**回读结果不等窗口**（低频且是身份/基线的
 * 权威来源）。
 *
 * ## 单订阅、跨页面
 *
...
/** 增量合帧窗口（毫秒）= 60Hz 显示刷新的 3 帧——攒批的收益在 2 帧后已基本到手，
 *  再长只会让「打字机」变顿。后端另有一层同类窗口（`Transcript::deliver`）。 */
 ...
 * 内部状态（globalThis 单例）。
 *
 * 理由与 `eventBus` 相同：Vite HMR 会重新执行本模块，模块级变量被
...
export function handleVdfsChange(change: VdfsChange): void {
  ...
  // ④ 状态帧（`llm_state_frame` 剥掉了正文）：本地已有该节点 ⇒ 正文就是权威的
...
function subscribeMount(mountDir: string, sink: TranscriptSyncSink): () => void {
  return subscribeVdfsChanged(
    ...
    () => {
      ...
      // 先丢弃待落地增量再重读：权威快照会覆盖它们，不丢就会把同一段正文追加两次。
      dropPending()
