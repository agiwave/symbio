 ...
 *
 * 1. **两种落点的语义边界**（`upsert` 整条替换 vs `append` 尾部追加）——判错会让
 *    内容重复拼接或倒退。语义由**协议操作**给出，不从节点 `type` / `role` 反推；
...
/**
 * 把一段增量追加到消息正文尾部（`append` 操作的**唯一**内容落点）。
 *
 * ## 为什么不再按节点类型猜"追加还是替换"
 *
 * 这里原本是 `mergeMessagePatch`：它从 `type` / `role` 反推语义——`tool_call` 与
 * `role = tool` 被判成"流式帧全量重发"因而**整条替换**，其余才追加。那条规则赖以
 * 成立的前提已经不存在：帧的语义由**操作**给出（`upsert` = 完整快照整条替换，
 * `append` = 尾部追加），后端对工具参数（`emit_append`，与 Text / Reasoning 同构）
 * 与工具响应（`tool_executor` 透传子会话的 `Append`）发的都是窄增量。
