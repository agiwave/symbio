...
import * as ShellExecute from '../schemas/shell_execute'
...
const executable = computed(() => {
  ...
  return ['python', 'python3', 'py', 'bash', 'sh', 'shell'].includes(lang)
})
...
async function executeCode() {
  ...
  try {
    ...
    if (['python', 'python3', 'py'].includes(lang)) {
      command = `python3 -c ${JSON.stringify(props.code)}`
    } else {
      command = props.code
    }

    const res = await callPlugin<ShellExecute.Response, ShellExecute.Request>('agent/shell/execute', {
      command
    })
