 ...
 *
 * 数据地址（`<根>/<rel>`）与浏览器地址（`/vdfs/<rel>`）是两个概念，换算是
 ...
 *    「相对路径」，**空段不产生空路径段**（`['session','']` ≠ `<根>/session/`）。
 * 2. 非虚拟地址**一律兜底 `/vdfs`**：拼错的地址会静默打开一个空页面，
 *    宁可回首页也不能拼进 URL。前缀陷阱（`<根>XXX`）同样落到兜底。
...
import { setVdfsRoot } from '../vdfsRoot'
...
describe('vdfsAddrOf — 路由参数 → 数据地址', () => {
  it('缺省 / 空串 → 根', () => {
    expect(vdfsAddrOf(undefined)).toBe(ROOT)
    expect(vdfsAddrOf('')).toBe(ROOT)
    expect(vdfsAddrOf(null)).toBe(ROOT)
    expect(vdfsAddrOf([])).toBe(ROOT)
  })

  it('单级字符串', () => {
    expect(vdfsAddrOf('session')).toBe('@vfs/session')
  })
...
describe('vdfsBrowserPathOf — 数据地址 → 浏览器地址', () => {
  it('根 → /vdfs', () => {
    expect(vdfsBrowserPathOf(ROOT)).toBe('/vdfs')
  })
  ...
  it('前缀陷阱：根名加后缀不算在根之下', () => {
    expect(vdfsBrowserPathOf('@vfssomething')).toBe('/vdfs')
    expect(vdfsBrowserPathOf('@vfs2/session')).toBe('/vdfs')
  })
