...
//! [`crate::symbio_core::plugin_dir`]）。所以声明里装的就是设置页列这一项所需的
//! 全部信息，而它们**只有拥有者说得出来**：
//!
...
///
/// 传入插件自己的 [`ConfigFile`]，条目形状由本函数统一构造（见 [`entry_of`]）。
/// 收集器不存在时静默跳过：本通道是**增益**，没有它插件照常工作，
/// 只是设置页列不出它。收集器由容器在广播前放进 `ctx`，见
/// `plugins/composite/vdfs.rs::children_of`。
pub async fn announce_configurable(ctx: &Arc<dyn InvokeRequest>, config: &ConfigFile) {
