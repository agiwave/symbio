//! `symbio/src/symbio_core/plugin/error.rs` 的单元测试 —— 拆自源码末尾的测试模块。
//!
...
fn error_code_roundtrip_through_frame() {
    let cases = [
        ...
        (
            PluginError::RateLimited("x".into()),
            PluginErrorCode::RateLimited,
        ),
        (
            PluginError::ParseError("x".into()),
            PluginErrorCode::ParseError,
        ),
        (PluginError::Timeout, PluginErrorCode::Timeout),
        (
            PluginError::Forbidden("x".into()),
            PluginErrorCode::Forbidden,
        ),
        (PluginError::Aborted, PluginErrorCode::Aborted),
        (
            PluginError::RetryWithoutContextId,
            PluginErrorCode::RetryWithoutContextId,
        ),
        (
            PluginError::StreamError("x".into()),
            PluginErrorCode::StreamError,
        ),
        (
            PluginError::CompressionFailed,
            PluginErrorCode::CompressionFailed,
        ),
    ];
...
fn unknown_or_missing_code_is_none() {
    assert_eq!(
        PluginErrorCode::from_code("NO_SUCH_CODE"),
