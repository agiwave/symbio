...
impl PluginFrame {
    ...
    pub fn error_code(&self) -> Option<crate::symbio_core::PluginErrorCode> {
        let PluginFrame::Error(_, meta) = self else {
            return None;
        };
        let raw = meta.as_ref()?.get("code")?.as_str()?;
        let code = crate::symbio_core::PluginErrorCode::from_code(raw);
        match code {
            crate::symbio_core::PluginErrorCode::Unknown => None,
            other => Some(other),
        }
    }
    ...
    pub fn is_abort(&self) -> bool {
        self.error_code() == Some(crate::symbio_core::PluginErrorCode::Aborted)
    }
}
...
impl Clone for PluginSerializeData {
    fn clone(&self) -> Self {
        Self {
...
/// 于是那两处只能写 `Err`。
///
/// 进程内对象的透传走的是**上下文扩展桶**（`PluginInvokeRequest::set_raw`，见
/// `PluginSimpleRequest::extensions` 的注释），与载荷枚举无关。两者职责不同：
...
impl PluginPayload {
    ...
    pub fn new<T: Serialize + Clone + Send + Sync + 'static>(data: &T) -> Self {
        Self::Data(PluginSerializeData::new(data))
    }
...
impl fmt::Debug for PluginPayload {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Data(_) => write!(f, "Data(PluginSerializeData)"),
