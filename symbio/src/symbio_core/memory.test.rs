...
fn cut_keeps_short_text_intact() {
    let m = InjectedMemory::cut("abc", 3);
...
fn body(text: &str, truncated: bool) -> InjectedMemory {
    InjectedMemory {
        text: text.to_string(),
        truncated,
