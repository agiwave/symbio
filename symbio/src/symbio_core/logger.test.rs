...
fn parse_level_maps_names_and_rejects_junk() {
    ...
    assert_eq!(parse_level("trace"), Some(LEVEL_DEBUG));
    assert_eq!(parse_level("off"), Some(LEVEL_ERROR));
...
fn gate_is_info_by_default_and_opens_for_debug() {
    ...
    assert!(!level_enabled(LEVEL_DEBUG), "默认不得输出 debug");
    assert!(level_enabled(LEVEL_INFO), "默认必须输出 info");
    assert!(level_enabled(LEVEL_WARN));
    assert!(level_enabled(LEVEL_ERROR));

    set_min_level(LEVEL_DEBUG);
    assert!(level_enabled(LEVEL_DEBUG), "放开后 debug 应可见");

    // 更严的级别（error）不受 debug 放开影响：error 仍可见，info 被挡
    set_min_level(LEVEL_ERROR);
    assert!(!level_enabled(LEVEL_INFO));
    assert!(!level_enabled(LEVEL_DEBUG));
    assert!(level_enabled(LEVEL_ERROR));

    // 复位：避免污染同二进制内的其它用例
    set_min_level(LEVEL_INFO);
    assert_eq!(min_level(), LEVEL_INFO);
}
