...
async fn full_channel_keeps_subscription() {
    ...
    EventBus::try_publish(KIND_VDFS, None, json!({ "n": 1 })); // 填满容量 1
    EventBus::try_publish(KIND_VDFS, None, json!({ "n": 2 })); // Full
...
async fn closed_channel_is_unregistered() {
    ...
    EventBus::try_publish(KIND_VDFS, None, json!({}));
...
fn resync_marker_is_a_bus_event_without_path() {
    let PluginFrame::Data(v) = resync_marker(KIND_VDFS) else {
        panic!("resync 标记必须是 Data 帧");
    };
    assert_eq!(v["type"], "bus_event");
    assert_eq!(v["data"]["kind"], KIND_VDFS);
    assert_eq!(v["data"]["data"]["type"], RESYNC_MARKER_TYPE);
    assert!(
...
async fn full_channel_eventually_receives_resync_marker() {
    ...
    EventBus::try_publish(KIND_VDFS, None, json!({ "n": 1 }));
    EventBus::try_publish(KIND_VDFS, None, json!({ "n": 2 })); // Full → 触发补送
    ...
    loop {
        match tokio::time::timeout_at(deadline, rx.recv()).await {
            Ok(Some(PluginFrame::Data(v))) if v["data"]["data"]["type"] == RESYNC_MARKER_TYPE => {
                saw_marker = true;
                break;
            }
            Ok(Some(_)) => continue,
...
async fn fan_out_shares_one_payload_allocation() {
    ...
    EventBus::try_publish(KIND_VDFS, None, json!({ "tag": tag }));
