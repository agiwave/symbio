...
fn truncate_does_not_panic_on_multibyte_boundary() {
    ...
    for n in 0..s.len() {
        ...
        let out = text_truncate_bytes(&s, n, "…");
...
fn truncate_shorter_than_limit_returns_original() {
    assert_eq!(text_truncate_bytes("hello", 10, "…"), "hello");
    assert_eq!(text_truncate_bytes("hello", 5, "…"), "hello");
}

#[test]
fn truncate_exact_boundary_no_suffix_when_equal_len() {
    // len == max_bytes 视为未截断（避免把刚好合规的内容也打省略号）
    assert_eq!(text_truncate_bytes("héllo", 6, "…"), "héllo");
}

#[test]
fn truncate_ascii_at_limit_appends_suffix() {
    assert_eq!(text_truncate_bytes("abcdef", 3, "..."), "abc...");
}
...
fn floor_char_boundary_never_splits_char() {
    let s = "中文abc";
    for n in 0..=s.len() {
        let b = text_floor_char_boundary(s, n);
        assert!(s.is_char_boundary(b), "n={n} 得到非法边界 {b}");
        assert!(b <= n);
    }
    assert_eq!(text_floor_char_boundary(s, 1), 0); // '中' 占 3 字节 → 回退到 0
    assert_eq!(text_floor_char_boundary(s, 2), 0);
    assert_eq!(text_floor_char_boundary(s, 3), 3);
    assert_eq!(text_floor_char_boundary(s, 999), s.len());
}

#[test]
fn zero_budget_is_safe() {
    assert_eq!(text_truncate_bytes("中文", 0, "…"), "…");
    assert_eq!(text_floor_char_boundary("中文", 0), 0);
}
