...
| 域 | 职责 | 关键符号 | 子路径 |
|---|---|---|---|
| `assembly` | 装配策略：一棵标准插件树挂哪些插件、哪些插件不许被停用 | `SUB_AGENT_PLUGINS` · `UNDISABLABLE_PLUGINS` | — |
...
| `keys` | 类型安全上下文键：键 trait + 键实例 + 插件 id + 路由地址常量 | `SymbioKey` 及其实例（`PATH` · `WORKDIR` · `ID` · `NAME` · `CAPABILITY_VISITOR` …）· `PLUGIN_*` · `EMBEDDING_*` · `KEY_PAYLOAD` · `<PLUGIN>_<OP>` 地址常量 | `ids` · `paths` |
...
| 键面常量 | `keys` 的 `KEY_*` / `RESERVED_KEYS` / `PLUGIN_FILE` / `PLUGIN_*` / `EMBEDDING_*`；`keys::paths` 的地址常量 | 线上键名与 id 是**跨进程字面量**，改的是值不是名字 |
| 类型化上下文键 | `keys` 的 `PATH` · `WORKDIR` · `ID` · `NAME` · `KIND` · `SCOPE` · `CONTENT` · `DESCRIPTION` · `MODE` · `AGENT_ID` · `SESSION_ID` · `TRACE_ID` · `PROVIDER_ID` · `RISK_LEVEL` · `TOOL_CALL_ID` · `RESULT_MSG_ID` · `VDFS_PARENT_ADDR` · `PARENT` · `CONFIG` · `PLUGIN_DIR` · `REQUIRED_PLUGINS` · `CAPABILITY_VISITOR` · `OPTION_VISITOR` · `CONFIG_VISITOR` · `EVENT_SINK` · `ABORT_SIGNAL` | 见下方「两条命名规则」——它们是**实例**不是名字，加 `KEY_` 前缀会与字符串键名撞成一个前缀 |
