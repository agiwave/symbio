...
本目录是**契约层**：只放跨插件共享的 trait、协议类型、词表常量与纯工具函数，
**不放实现**。谁有资格进来由准入判据决定——**只被一个模块依赖的内容一律下沉回该模块**
（判据与理由见 [ADR-023](../../../docs/DECISIONS.md)）。本文是该目录**命名与结构**的唯一 owner。
...
3. **依赖单向、只依赖更底层**：域之间只允许「上层 → 下层」，不得互相回指；插件专属的实现
   代码不进本层（本层只留契约与共享数据结构，如 `llm` 只留契约，HTTP 重试与 SSE 流循环
   留在 `plugins/model/`）。**唯一的宿主接缝形态是「纯接口 + 一个桥文件」**：接口与域类型
   宿主无关，只有 `host.rs` 知道本宿主的信封类型（`vdfs/host.rs` 是范本），换宿主只重写那一个文件。
...
2. **是契约还是实现？** 只有实现方认的东西（重试机器、流循环、拓扑知识）留在插件里。
