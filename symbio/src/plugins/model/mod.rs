...
//!   + 协议事件方言 `ModelProtocolEvent` + 行解析契约 `SseLineParser`（`protocols/sse`）
...
//! 协议抽象 `ModelProtocol`、事件方言 `ModelProtocolEvent`、行解析契约
//! `SseLineParser` 都是本插件私有词汇，core 不承载它们——core 只留
//! session 与 model **两侧共用**的 `ModelProvider` / `TurnOutput`。
