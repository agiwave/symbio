...
const PRESETS: &[PresetTuple] = &[
    ("openai", "OpenAI", "https://api.openai.com/v1",
     &["gpt-4o-mini", "gpt-4o", "gpt-4-turbo", "gpt-3.5-turbo", "o1", "o1-mini", "o3-mini"],
    ...
    ("gemini", "Google Gemini", "https://generativelanguage.googleapis.com/v1beta",
     &["gemini-2.5-pro", "gemini-2.5-flash", "gemini-2.0-pro-exp-02-05", "gemini-2.0-flash", "gemini-2.0-flash-lite-preview-02-05", "gemini-2.0-flash-thinking-exp-01-21"],
     &["gemini_api"]),
    ...
    ("siliconflow", "硅基流动 (SiliconFlow)", "https://api.siliconflow.cn/v1",
     &["deepseek-ai/DeepSeek-R1", "deepseek-ai/DeepSeek-V3", "Qwen/Qwen2.5-72B-Instruct"],
     &["openai_chat"]),
    ("alibaba", "阿里云百炼 (Qwen)", "https://dashscope.aliyuncs.com/compatible-mode/v1",
    ...
    ("tencent", "腾讯混元", "https://api.hunyuan.cloud.tencent.com/v1",
     ...
     &["openai_chat"]),
    ("baidu", "百度千帆 (ERNIE)", "https://qianfan.baidubce.com/v2",
    ...
    ("zhipu", "智谱 (GLM)", "https://open.bigmodel.cn/api/paas/v4",
     &["glm-4.7-flash", "glm-4-plus", "glm-4-flash", "glm-4", "glm-3-turbo"],
     &["openai_chat"]),
    ("aiyuanjing", "爱媛景 (GLM 兼容)", "https://maas-api.ai-yuanjing.com/openapi/compatible-mode/v1",
     &["glm-5", "glm-4-plus", "glm-4"],
     &["openai_chat"]),
    ("lmstudio", "LM Studio（本地）", "http://localhost:1234/v1",
     &[],
     &["anthropic_messages", "openai_chat", "openai_responses"]),
    ("local", "Ollama（本地）", "http://localhost:11434/v1",
    ...
    ("mistral", "Mistral", "https://api.mistral.ai/v1",
     ...
     &["openai_chat"]),
    ("azure", "Azure OpenAI", "https://<your-resource>.openai.azure.com/openai/v1",
     &[],
     &["openai_chat"]),
    ...
    ("volcengine", "火山方舟 (豆包)", "https://ark.cn-beijing.volces.com/api/v3",
     &["doubao-seed-1-6-250615", "doubao-1-5-pro-32k-250115", "doubao-1-5-lite-32k-250115"],
     &["openai_chat"]),
    ...
    ("baichuan", "百川智能", "https://api.baichuan-ai.com/v1",
     ...
     &["openai_chat"]),
    ("step", "阶跃星辰 (StepFun)", "https://api.stepfun.com/v1",
...
fn preset_from(t: &PresetTuple) -> DetailPreset {
    ...
    if let Some(first_protocol) = protocols.first() {
        set_always.insert("api_protocol".to_string(), serde_json::json!(first_protocol));
    }
    let mut options = std::collections::BTreeMap::new();
    options.insert("model".to_string(), models.iter().map(|s| s.to_string()).collect());
    options.insert("api_protocol".to_string(), protocols.iter().map(|s| s.to_string()).collect());
    DetailPreset {
        value: value.into(),
        label: label.into(),
        set,
...
pub fn model_detail_definition() -> DetailDefinition {
    let provider_options: Vec<DetailOption> =
        PRESETS.iter().map(|t| opt(t.0, t.1)).collect();
    ...
    DetailDefinition {
            ...
            DetailSection {
                ...
                fields: vec![
                    DetailField {
                        ...
                        ..field("name", "名称", "未填写时自动使用模型名称（ID 自动生成，无需填写）", "text")
                    },
                    DetailField {
                        options: provider_options,
                        ..field("provider", "提供商", "选择一个内置的 Model 提供商预设", "select")
                    },
                    DetailField {
                        required: true,
                    ...
                    DetailField {
                        ...
                        ..field("api_base", "API Base URL", "切换提供商时会自动填入对应端点", "text")
                    },
                    DetailField {
                    ...
                    DetailField {
                        ...
                        ..field("enabled", "启用", "禁用后该 Provider 在 Model 对话选项中不可选", "toggle")
                    },
                ],
            },
            DetailSection {
                ...
                fields: vec![
                    DetailField {
                        ...
                        ..field("api_protocol", "API 协议", "与该 Provider 通信的协议风格", "select")
                    },
                    DetailField {
                        min: Some(0.0),
                        max: Some(2.0),
                        ...
                        ..field("temperature", "Temperature", "控制输出随机性（0 - 2）", "number")
                    },
                    DetailField {
                        min: Some(100.0),
                        max: Some(128000.0),
                        ...
                        ..field("max_tokens", "Max Tokens", "单次回复最大 token 数（留空使用默认）", "number")
                    },
                    DetailField {
                        min: Some(0.0),
                        step: Some(100.0),
                        ...
                        ..field("rate_limit_ms", "请求频率限制（毫秒）", "两次请求之间的最小间隔；0 表示不限制", "number")
                    },
                    DetailField {
                        rows: Some(3),
                        full_width: true,
                        placeholder: Some("可选：默认 system prompt".into()),
                        ..field("system_prompt", "系统提示词", "应用于此 Provider 的全局系统提示词", "textarea")
                    },
                ],
            },
        ],
        ...
        badges: vec![
            DetailBadge {
                when: Some(DetailCondition { key: "is_default".into(), equals: Some(serde_json::json!(true)), ..Default::default() }),
                label: "默认".into(),
                style: "default".into(),
            },
            DetailBadge {
                when: Some(DetailCondition { key: "enabled".into(), equals: Some(serde_json::json!(false)), ..Default::default() }),
                label: "已停用".into(),
                style: "disabled".into(),
            },
        ],
        actions: vec![
            DetailAction {
                ...
                when: Some(DetailCondition { key: "cap.test_connection".into(), equals: Some(serde_json::json!(true)), ..Default::default() }),
                disabled_when: Some(DetailCondition { key: "is_existing".into(), equals: Some(serde_json::json!(false)), ..Default::default() }),
            ...
            DetailAction {
                ...
                when: Some(DetailCondition {
                    all: vec![
                        DetailCondition { key: "is_default".into(), equals: Some(serde_json::json!(false)), ..Default::default() },
                        DetailCondition { key: "is_existing".into(), equals: Some(serde_json::json!(true)), ..Default::default() },
                    ],
                    ..Default::default()
                }),
                ..Default::default()
...
mod tests {
    ...
    fn definition_covers_model_form_surface() {
        ...
        assert!(ids.contains(&"test") && ids.contains(&"save") && ids.contains(&"delete") && ids.contains(&"set-default"));
    }
}
