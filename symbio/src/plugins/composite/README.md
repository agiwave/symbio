...
- **VDFS 拓扑唯一持有者**：逐子插件广播收集各自的 `impl VdfsProvider`，组合成「包含子目录列表的 provider」（`CompositeVdfs`）并登记进 `register_vdfs_root` 槽位，成为 `<根>` 的服务者——它没有任何「根级别」概念（见 `docs/design/vdfs.md` §2.5 / §13.2）。
