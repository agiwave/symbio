...
// ==================== 收集器与收集管线（随机制从 core 下沉） ====================
//
// 这三条原在 `symbio_core/capability/option.test.rs`，随 `collect_options` /
// `DefaultOptionVisitor` 一起搬来。测的是**宿主侧装配**（排序 / 去重 / 无父插件
// 降级），不是契约——契约由 `OptionVisitor` 的文档钉住。
