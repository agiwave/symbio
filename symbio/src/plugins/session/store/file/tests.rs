...
async fn sub_session_routes_into_parent_dir() {
    ...
    save(&store, &session_with("v2_sess_child", Some("v2_sess_parent"))).await;
    ...
    assert!(store.list_sub_sessions("v2_sess_noparent").await.unwrap().is_empty());
    ...
    assert_eq!(
        ...
        tmp.path().join("v2_sess_parent").join("sessions").join("v2_sess_child")
    );
    store.delete_session("v2_sess_child").await.unwrap();
    assert!(store.list_sub_sessions("v2_sess_parent").await.unwrap().is_empty());
}
...
async fn deleting_parent_cascades_sub_sessions() {
    ...
    let store = FileSessionStore::new(tmp.path().to_path_buf());
    save(&store, &session_with("v2_sess_parent", None)).await;
    save(&store, &session_with("v2_sess_child1", Some("v2_sess_parent"))).await;
    save(&store, &session_with("v2_sess_child2", Some("v2_sess_parent"))).await;
