...
>
> **状态：S16–S22 已完成；S23–S25 消息与会话运行态曾改为单条转写流；
> S26（ADR-025，2026-09-23）实时面迁回 VDFS 变更。**
...
>   | `change` | `delta` | 消费端动作 |
>   |---|---|---|
>   | `created` | — | 插入该节点（回读拿全貌：身份 + `seq` + 正文） |
>   | `updated` | **有** | **尾部追加**，零回读 |
>   | `updated` | — | 节点变了，回读（`stat` / `read`） |
>   | `deleted` | — | 就地移除 |
>   | `created` / `deleted` 带 `delta` | — | **协议违例**（报错丢弃，不占任何东西） |
>
>   **取值集合不变**（仍是三个），只加一个可选字段。**「追加」不是新取值**——它是
>   `updated` 上的 `delta`。**顺序是节点属性**（`ChatMessage.seq`），与到达顺序无关：
>   两个并行工具的变更**混着到**、后生成的**先到**，显示都正确——因为每条变更都指向
>   一个明确的 `path`，而节点自带位置。
>
> **S23–S25 那段（单条转写流）已整体退役**：`session/stream` 路由、
> `symbio_core::transcript_stream`、`services/transcriptStream.ts` 一并删除。
> 它存在的三条理由（**顺序** / **背压** / **免回读**）现在都不成立——顺序不是投递属性
> （见上）；`event_bus` 已有**对等**背压（满通道保留订阅 + 补送 resync 指令，`ea8a460`）；
> 免回读由 `delta` 直接提供。**旧的 `kind = "session"` 事件频道
> （`Status` / `Update` / `Delete` / `Error` / `Abort`）仍然废除**，不再有第三种通道。
>
> ⚠️ **§3.1 / §3.2 / §4 / §5 描述的是 S16–S22 的「VDFS 变更」模型**（`created` /
> `appended` / `updated` / `deleted` / `truncated`，消费端 `services/vdfsTranscriptSync.ts`）。
> 那个文件**已被删除**；S23–S25 曾把消息挪出 VDFS，**S26 又挪了回来**——但形态变了：
> **没有 `appended`，只有 `updated` + `delta`**。这几节保留为**设计推导的历史记录**，
> 实时面的权威描述在 [`docs/design/vdfs.md`](../../../../../docs/design/vdfs.md) §9 与
> `symbio_core/vdfs_provider.rs` 的「变更通知」小节。
> **仍然有效**的是：§2（节点分类 / 状态机）、§5.3（工具调用三段式）、§6 的 S20–S25
> 各阶段、§7（体验清单）、§8（不变量）、§11（批次 E，含 S26 后记）。
>
...
> **现状校正（批次 E + G，2026-09-22）**：本节（§3.1–§3.2）与 §4.1 描述的是 **S20
> 当时**的模型——会话域实时面走 `kind = "vdfs"` 一条频道、变更**按类型带载荷**
> （`node` / `content` / `delta`）。两批改动之后这两条前提都已不成立：
>
> - **批次 E**：会话运行态（与消息一起）移出 VDFS，改走 `session/stream` 转写流
>   （单通道保序 + 会话内单调 `seq`）。VDFS 只剩会话**资源**变更。
> - **批次 G**：`VdfsChange` 收窄为 `{ path, change }` 两个字段——`to` / `delta` /
>   `node` / `content` 四个载荷与 `renamed` / `appended` / `truncated` 三个取值
>   **整体删除**（它们从无生产性生产者，判据与历史对照表见
>   `docs/design/vdfs.md` 的词汇表、`schemas/vdfs.VdfsChange`）。
>
> 因此下面两张表读作**历史设计**；现行契约见 §5.1、§11 与
> `docs/design/vdfs.md` 的词汇表。
...
> **后记（批次 G，2026-09-22）：第 3 条已随取值一起消失。** `appended` 没有生产性
> 生产者，连同 `delta` 载荷整体删除（判据见 §3 的现状校正块）。前两条仍成立：
> 第 1 条现在只影响 VDFS 那条**幂等重拉**通道，第 2 条是 `refreshList` 的既有规则。
...
>
> **再后记（批次 G，2026-09-22）**：连「其他追加型资源」这条出路也没有了——
> `appended` 这个**取值本身**已从 `VdfsChange` 删除（无生产性生产者），`delta`
> 载荷随之删除。也就是说：**VDFS 变更频道上不存在也不该存在增量语义**，
> 任何增量都必须在承载它的资源的**有序流**上（会话消息即转写流）。
...
这条换法只对**状态**成立：增量不可能不依赖顺序。所以会话域现在**没有增量帧**
——消息帧带的是整条 `ChatMessage`（`delta` 由接收端累加，`content` 整条替换）。
热路径的代价从「解地址」变成「多带一点载荷」，换掉的是一条跨通道顺序假设。
（批次 G 之后连「VDFS 变更带增量载荷」这条退路也没有了：`appended` / `delta` 已删除。）

VDFS 那条通道上剩下的是**资源**变更（创建 / 删除 / 改名 / 标题 / metadata），
它们发生在**没有在途轮次**时，拿不到 `seq`，因此必须留在这里；收敛方式统一为
**重拉清单**（幂等 ⇒ 无序无害）——变更只有 `path` + `change` 两个字段，**没有载荷
可解读**（见 `stores/sessionNodeSync.ts` 模块文档「为什么这里的变更不带节点快照」）。
...
10. **实时面走 VDFS 变更，两条事实各是一个节点**（S26 / ADR-025）：消息**是**
    ...
    **VDFS 变更通道因此是唯一的实时通道**：既不携带会话节点快照（快照会**陈旧**，
    而回读永远最新且幂等），也不需要第二种通道。
    判断「本轮 / 子会话结束」看会话节点的 `status`（不再有 `Status idle` 帧可等），
    **不是**根 Turn 的终态（一轮里它会多次定格）。旧事件频道（`kind = "session"`）
    已整体废除——**而不是**"前端不订、后端还发"。前端（`stores/sessionNodeSync.ts` /
    `composables/useVdfs.ts`）、CLI（`cli/src/client.rs`）与子智能体转播
    （`agent/host/subagent.rs`）三处消费者都订同一条 `KIND_VDFS` 变更。
...
20. **丢了怎么自愈**：`event_bus` 满通道时**不摘除订阅**——保留订阅并补送一条
    **resync 指令**（`data.data.type == "resync"`，`ea8a460`）；消费端收到即按自己的
    作用域**整份重读**（重读源是 VDFS 读面，权威且幂等，无需补帧）。这是**唯一**一层，
    消息与运行态**共用**（S26 前是「两条通道各有承诺」，那条分治随 `session/stream`
    退役而消失）。因此消费端不得把"没收到终态"就地把节点猜成 `completed`——服务端
    可能定稿成 `aborted` / `failed`，猜错就是"把半截谎报成正常结束"。
...
### 11.7 S26 后记（ADR-025）：本节的推理已被纠正，方向已反向

本节（批次 E）把会话运行态并入转写流，理由是「与消息**共用 `seq` 空间**，否则
『会话不忙 ⇒ 本轮节点已终态』不成立」。**这条推理的前提是错的**——它假设了
「顺序是投递属性」。顺序是**节点属性**（`ChatMessage.seq`，消费端按它排序），
因此到达顺序与显示顺序无关，那条「顺序保证」**无从需要**。

具体纠正三处：

1. **§11.4 的「快照的来源必须有序或幂等」——只有「幂等」那一半是对的。**
   「有序」把**陈旧写入**误诊成了**乱序**：单写入者 + 单通道 FIFO 下不存在
   「后到的是旧的」；真正的危险是「快照携带的是它**生成那一刻**的状态」。
   所以不加快照的理由不是「通道无序」，而是「**快照会陈旧，回读永远最新且幂等**」。
   结论（不加快照）不变，**理由换了**。
2. **运行态不必与消息共用 `seq` 空间**：它是会话节点（`<根>/session/<sid>`）的
   `status`，一条 `updated`（无 `delta` ⇒ 回读）即可。§11.6 那条「运行态帧不走合帧
   窗口」的理由随之消失（合帧窗口本身也随转写流一起退役）。
3. **§11.5 的改动落点表描述的是批次 E 的方向**，S26 把它整体**反向**：
   `transcript_stream.rs` 删除、`Transcript::emit_session_state` 改为
   `change_subs.notify(<sid>, "updated")`、前端回到 `subscribeVdfsChanged`、
   CLI 回到 `event_bus/subscribe` + `vdfs/watch`、`subagent.rs` 的转播桥同理。

**判据不变**（一个变更取值 / 载荷字段必须有**生产性生产者**）：批次 G 删除 `delta`
时它确实零生产者（消息域已迁走）；S26 把消息域搬回来，**就有了**。
结论不同是因为**输入不同**，不是反复。
...
- 快照仍然**不带**会话正文（`content`）：`read(<根>/session/<sid>)` 才是历史读入口。
  ...
  `transcript_session`"。
- **订阅是全局广播，订阅表没有会话过滤位**：`handle_stream_subscribe` 的形参是
  `_ctx`（连上下文都不读），`fan_out` 把每帧 `clone()` 后交给**全部**订阅者，归属由
  帧里的 `session_id` 给出。这是**有意**的——CLI 靠它「订阅一次覆盖所有会话、切会话
  不必重连」（`cli/src/client.rs` 有原话），前端靠它显示后台会话进度（`S.pending` 按
  `sessionId` 分桶）。代价是**单会话消费端收全量帧**：目前唯一真正吃亏的是
  `agent/host/subagent.rs` 的转播桥（每个子会话一条订阅，且是**进程内直连**
  `register_transcript_subscriber`、不走路由），N 个子智能体并行时是 N 个订阅者 ×
  (N+1) 个会话的帧投递，投递还含克隆（带 `content` 的帧可达 KB 级）。
  > **已知的收窄方向（记录备将来，不是待办）**：给订阅加一个**可选**的会话作用域。
  > **不能改成必填**——那会让 CLI 的「切会话不必重连」与前端的多会话收敛一起退化。
  > 常见并发（1–3 个子智能体）下省下的量可以忽略，**等真的观察到帧投递成为热点再做**，
  > 不要为了「整齐」先做。顺序保证不受影响：`seq` 本来就是会话内单调。
