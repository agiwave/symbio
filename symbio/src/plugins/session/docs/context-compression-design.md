...
| 层 | 名称 | 作用域 | 手段 | 成本 |
|---|---|---|---|---|
| L0 | 单条工具结果守卫 | 单条 Tool 结果 | > 8192 tok → 存档 + head/tail 摘要（单行超长时 head/tail 再按字符截断兜底） | 0 |
| L1 | 单消息行数脱水 | 单条超长消息 | > 15 行 → 存档 + 末 10 行 + 路径脚注 | 0 |
| L2 | 轮内老化淡化（fade） | 单轮内历史工具结果 | > 40 工具轮激活，老结果压到 2k tok | 0 |
| L3 | 窗口骨架化 | 请求组装期 | > 15 个 ToolCall 明细 → 带语义摘要（规模 + 条目名/首行）+ 定位锚点回声 + 取回指引（`Re-run <tool>`）的占位符；声明 `context_retention` 的工具最新 N 次豁免窗口 | 0 |
...
> 各层的实现位置（文件/函数/常量）统一见 `../README.md` 策略③④⑥，不在本文重复维护。
...
1. **L0 守卫**：单条结果 > 8192 tok → 存档 + head/tail（尾部多为结论/错误更关键；单行超长时 head/tail 再按字符截断兜底，防止单行大 JSON 绕过）；
