...
use crate::symbio_core::PluginFactoryRegistry;
...
impl PluginFactory for HomeFactory {
    ...
    fn create(&self, _parent: Option<Weak<dyn Plugin>>, _config: Option<&Value>) -> Arc<dyn Plugin> {
        ...
        let home: Arc<dyn Plugin> = Arc::new_cyclic(|home_weak| {
            ...
            let note_config = global_config.plugins.get("note");
            let note = registry.get("note")
                .map(|f| f.create(Some(home_weak_dyn.clone()), note_config))
                .unwrap_or_else(|| Arc::new(crate::plugins::note::NotePlugin::new(Some(home_weak_dyn.clone()))));
            ...
            // 创建 Explorer（使用默认的空事件发送器，Tauri 层会重新创建并注入）
            ...
            let explorer = registry.get("explorer")
                ...
                .unwrap_or_else(|| Arc::new(crate::plugins::explorer::ExplorerPlugin::new(Some(home_weak_dyn.clone()), crate::symbio_core::event::OptionalEventSender::new(None))));

            // 创建 Agent，传入配置
            let agent_config = global_config.plugins.get("agent");
            let agent = create_agent_with_factory(home_weak_dyn, agent_config);

            HomePlugin::new_with_config(work.clone(), note.clone(), agent, setting.clone(), explorer.clone(), global_config.clone())
        });
