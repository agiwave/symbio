...
fn memory_is_the_workspace_root_agents_file() {
    ...
    assert_eq!(MEMORY_VDFS_ADDRESS, ".vdfs/work/AGENTS.md");
}
...
fn no_workspace_means_no_scope() {
    ...
    assert_eq!(m.segment(&segment_spec()).unwrap(), None);
...
fn segment_spec_is_the_workspace_layer_personality() {
    ...
    assert_eq!(s.address, MEMORY_VDFS_ADDRESS);
...
fn segment_round_trips_through_the_real_file() {
    ...
    let seg = m.segment(&segment_spec()).unwrap().unwrap();
    assert!(seg.contains("【工作区记忆】"));
    assert!(seg.contains(MEMORY_VDFS_ADDRESS), "地址要下发: {seg}");
