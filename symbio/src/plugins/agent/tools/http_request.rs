...
impl HttpRequestTool {
    ...
    fn validate_url(&self, url: &str) -> Result<String, PluginError> {
        ...
        if !url.starts_with("http://") && !url.starts_with("https://") {
            return Err(PluginError::ValidationError("只允许 http:// 和 https:// URL".to_string()));
        }
        ...
        if is_private_or_local_host(&host) {
            return Err(PluginError::ValidationError(format!("阻止本地/私有主机: {}", host)));
        }

        // 检查允许的域名
        if !host_matches_allowlist(&host, &self.allowed_domains) {
            return Err(PluginError::ValidationError(format!(
                "主机 '{}' 不在 allowed_domains 中",
                host
            )));
        }
    ...
    fn validate_method(&self, method: &str) -> Result<reqwest::Method, PluginError> {
        match method.to_uppercase().as_str() {
            ...
            _ => Err(PluginError::ValidationError(format!(
                "不支持的 HTTP 方法: {}。支持: GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS",
                method
            ))),
        }
    }
