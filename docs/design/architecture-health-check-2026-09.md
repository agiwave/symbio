...
> `serde(rename_all = "snake_case")` 枚举 ↔ 前端词表数组，4 张）与 **D 组结构体
> 字段**（9 对，比对字段名；上线当次抓出 `ChatMessage` 的 `agent_id` / `prompt`
> 两个死字段）。决策与理由见 `docs/DECISIONS.md` **ADR-019**；
> `protocol-mirror-audit.test.mjs` 的回归测试同步从 9 条扩到 **30 条**。
> 下方原文保留作问题描述。
