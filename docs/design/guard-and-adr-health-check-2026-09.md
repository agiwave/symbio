...
### 实施状态（2026-09-20 当天跟进）

| # | 状态 | 落地方式 |
|---|---|---|
| 1 | ✅ | CI 新增提交信息校验步骤（PR / push 两种事件各自算范围），**并**装了本机 `core.hooksPath`；`check-commit-msg.mjs` 新增 `--range` 模式 |
| 2 | ✅ | 三个守卫各补一个 `*.test.mjs`（29 条），接进 gate 的回归测试循环；顺带修了 `style-audit` 的 `walk` 空目录崩溃与 `doc-link-audit` 默认永不失败 |
| 3 | ✅ | 改用 `paths-ignore`（只排除产出目录）；实测那些目录**都不被 git 跟踪**，故等价于"跟踪文件的改动一律触发" |
| 4 | ✅ | C 组扩展出**常量组**这一种后端写法并登记 `OPTION_PICK_*` ↔ `OPTION_PICKS`；前端词表收成唯一定义处；Rust 侧那条不成立的豁免理由已改写为如实描述 |
| 5 | ✅ | 删除 `SchemaResponse`（struct + re-export）；并让死项报告区分「已承认保留」与真可删项 |
| 6 | ✅ | 已加作废标记 |
| — | ✅ | **跟进中新发现**：8 条 `Corresponding Frontend` 头里 **7 条指向不存在的文件**（6 条指向从未在版本史里出现过的 `tauri/src/protocols/`）⇒ 删掉悬空头，并在跨栈守卫加 **E 组**判定 |
