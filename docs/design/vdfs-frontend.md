...
> **现状校正（针对 §7.1 的 S3，含 S25 / 批次 G / ADR-025 更新）**：§7.1 记「消息（转写）不经 VDFS」，
> 那是当时的形态。后来消息**读面**迁到了 VDFS——`<根>/session/<id>/message/<mid>`，
> 节点 `ext = message`，详情由 `components/vdfs/VdfsMessageDetail.vue` 呈现。
> **实时面也回到了 VDFS**（[ADR-025](../DECISIONS.md#adr-025-顺序是节点属性delta-是updated的传输形态)，2026-09-23）：
> 消息**是** `<根>/session/<id>/message` 这个**文件夹**里的**文件**，流式输出是该文件内容的
> **增长**——发 `updated` + `delta`。会话运行态是会话节点（`<根>/session/<id>`）的 `status`，
> 同样走 `updated`（无 `delta` ⇒ 回读）。于是 `kind = "vdfs"` 是**唯一**实时通道。
> §7 是进度档案、按约定不改写，但读 §7.1 时请以本条为准。
>
> **历史（被纠正的两步）**：S23–S25 曾把消息与会话运行态移出 VDFS、改走 `session/stream`
> 转写流，理由是「VDFS 变更频道**没有流内序号**（丢帧不可检测）、载荷全量而消费端要
> **猜「追加还是替换」**」。这两条**把「数据的属性」当成了「传输的属性」**：顺序是
> **节点属性**（`ChatMessage.seq`，消费端按它排序），「追加」不是新取值而是 `updated` 上的
> `delta` 字段（有 = 尾部追加、无 = 回读）。批次 G（`8969ec2`）删除 `delta` 时它确实零生产者
> （消息域已迁走）；消息域搬回来，就有了。
>
> **批次 G 那条「本视图不随流式增长」随之作废**：`VdfsMessageDetail.vue` 现在**会**随
> `updated` + `delta` 就地增长——这正是「流式即文件追加」的可视化。当时说「不该通过 VDFS
> 让它增长，因为迟到帧会把正文回退」也**不成立**：会回退的是**全量快照**，而 `delta` 是
> **尾部追加**（单调、不可回退），两者不是一回事；真正要防的是「在途重读用旧快照覆盖本地
> 增量」，那由代际守卫解决（`appendGuard`，见 `useVdfs`）。
