# 实体提供者机制（EntityProvider）

状态：现行规范（**后端内部抽象**）
范围：各插件「实体」的存储 / 校验 / 容器语义实现层——即 VDFS 挂载点背后「资源怎么存、怎么校验」

关联：
- [vdfs.md](./vdfs.md)（VDFS 机制规范；本文件描述的是它 §11 / §13.4 所依赖的实体底座）
- [vdfs-frontend.md](./vdfs-frontend.md)（前端页面规范）
- `symbio/src/symbio_core/entities.rs` —— trait、注册表、统一操作实现
- `symbio/src/symbio_core/schemas/entities.rs` —— 数据结构权威定义
- `symbio/src/symbio_core/vdfs/entity_adapter.rs` —— `EntityVdfsAdapter`，本机制的**唯一消费者**

> **对外只有一个协议（VDFS）**：`entities/*` 调用协议已于 S11 下线，前端统一实体页
> 机制已于 S5 收口（前端资源页只有 VDFS 一台，见 [vdfs-frontend.md](./vdfs-frontend.md) §7）。
> 历史形态（含迁移期的端点映射表）见
> [archive/entity-management-mechanism.md](../archive/entity-management-mechanism.md)。
>
...
（[vdfs.md](./vdfs.md) §11）：适配器把它挂在节点 `schema` 上、`ext = form`，前端
通用表单渲染器据此动态生成详情页。
