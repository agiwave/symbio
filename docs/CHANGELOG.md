...
- **后端**：`chat_loop` 因工具失败退出循环（needs_user_action）时，给触发的 Failed ToolCall 打 `meta.recoverable=true` 并广播+持久化——服务端唯一真相，区分「信息性失败」与「待恢复失败」
- **前端**：重试/补充参数按钮仅对 `recoverable` 失败显示；可恢复失败自动展开（需要操作入口），运行中失败保持单行红标（可手动展开看错误，无按钮）
- **设计取向**：长期看 `supply` 应与 ask_user/user_prompt 结构化提问统一（参数修复走表单而非 JSON 补丁），保留但视觉降级
