...
是**流式增量**，一次回复的帧数由模型决定。实测一段约 200 字的回复产生 **580 行**
`[T#N Append] <id> - +1c`，占该轮 stderr 的 **93%**（622 行里 587 行是帧日志）——
时间线被同一件事填满，"工具什么时候开始跑的"要靠 `grep` 才找得到。

**改动**：

1. 新增 `AppendLogCoalescer`（`session/transcript.rs`）：相邻的、**同 `message_id`**
   的 `Append` 并入同一个 run，不逐帧产出日志行；换 id、换操作、或显式冲刷
   （`persisted` / `clear` / `Warn`）时把 run 渲染成一行。
   - 单帧：`[T#7 Append] a - +3c`（与折行前**逐字相同**）
   - 多帧：`[T#4..473 Append] a - 470 帧 / +470c`（带 seq 区间 / 帧数 / 累计字符）
...
- `node scripts/gate.mjs` **32 / 32**；`cargo test --lib` **887 passed / 0 failed**
  （881 → 887，+6）。
- 实测（`--provider LMStudio`，同一提示词）：stderr **622 → 44 行**，帧日志
  **587 → 9 行**。折行后的时间线一眼可见结构：推理节点 325 帧 / +1536c，
  正文节点 282 帧 / +479c。
- **CLI 端到端（确定性）**：`mock-chat-split` 场景（单行 809 字节、逐字节写出）
  stdout 与期望文本**逐字节一致**（627 字符）；470 个增量帧折成 1 行，`seq` 连续无缺口。
