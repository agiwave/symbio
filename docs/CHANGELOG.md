...
## 2026-09-22: 门禁改造 —— `fmt` / `gen-current-facts` 从「检查项」改为「门禁自动执行的工作」

**问题**：这两件事一直是**检查项**——门禁跑 `cargo fmt --all -- --check` 与
`gen-current-facts --check`，不一致就判红，然后等人去跑一次修复命令再提交。
但格式化与事实文件生成是**函数，不是判断**：`fmt(code) → code'`、`gen(code) → facts`
对同一份输入永远给同一个输出。把它们写成检查，等于让门禁因为**人忘了按一次按钮**而红
——它报的不是代码有问题，是流程有问题。而修复动作完全确定、零风险，没有任何理由等人来按。
（此前甚至有一个 `--fix` 开关来「顺便修一下」，但它**修完仍然跑 `--check`**，
且改完 `grep ctx.fix` 已无任何使用者——一个没有任何效果的开关。）

**改法**：门禁新增原语 `gate.d/_shared.autoWork`——把一件确定性机械工作**做掉**，
而不是判它「有没有做过」。语义：

- 命令**跑成功** ⇒ 通过（产物被改写不算失败，那是它该做的事）；
- 命令**本身报错** ⇒ 不通过（工具坏了 / 输入不可解析）；
- **本地**：被改写的路径**当场暂存**，使修复与「本次提交」是同一份内容；
- **CI**（`--ci`）：CI 不能提交，所以「执行后仍有差异」只能报红。

| 层 | 改动 |
|---|---|
| `scripts/gate.d/_shared.mjs` | 新增 `autoWork`（含 `dirtyPaths` / `contentHashes`）；`--fix` 语义整体删除 |
| `scripts/gate.d/10-backend.mjs` | 两处 `cargo fmt --all -- --check` → **无条件自动执行**，且**移到各自阶段最前**（否则报的是未格式化代码的问题） |
| `scripts/gate.d/60-facts.mjs` | 整文件重写为单一 `autoWork` 任务 |
| `scripts/gate.mjs` | 删 `--fix`；头部文档改为「判定 vs 执行」 |
| `scripts/commit.mjs` | 删 `--fix` 透传；**门禁后会改索引**，故第 2 步改为**重读** git 状态（不再沿用门禁前的快照），并加「门禁后索引为空」硬校验 |
| `scripts/gate.d/30-docs.mjs` | `TEST_ONLY` 纳入 `gate.d/_shared`（新回归测试接进门禁） |
| `CONTRIBUTING.md` | 删 `--fix`；写明自动执行语义与 `--ci` 耦合 |
| `scripts/gen-current-facts.mjs` | **改为幂等**（见下） |

**⚠️ 自查出的两个缺陷（都是这次改动自己引入的）**：

1. **`autoWork` 第一版把「被修复改写的路径」判反了方向。** 第一版按「修复前干净、
   修复后变脏」判定要暂存的路径。但日常流程是「改文件 → `git add` → 提交」，
   所以**修复前就已脏（甚至已暂存）才是常态**——那样会把它们全部排除，
   于是**提交里留下的是未格式化那一版**。正确判据是**内容哈希**：修复前给所有脏路径
   记 sha256，修复后重算，哈希变了就是被改写过（无论它此前是干净、已暂存、还是已有
   未提交改动）。「修复前就脏、但修复没碰」的路径**不会被暂存**——门禁没有立场替人
   决定那些改动该不该进本次提交。
2. **`docs/CURRENT.md` 含生成时间戳（与当前 HEAD sha），内容每次运行都变** ⇒ 按内容哈希
   比对时，门禁**每次**都会报「修复了 1 个文件」，而 CI 无法提交、只能判红，
   **红的原因是时间，不是漂移**。修法不是绕过比对，而是让生成器**真正幂等**：
   代码派生内容（除溯源行外）没变时整份沿用已有文件（连同它原来的溯源行）。
   溯源行因此表示「**内容最后一次真正变化**是在何时、基于哪个 commit」，比
   「最后一次运行时间」更有意义。三次连跑同一 blob（`da0aaa0…`）已验证。
   同时 `ci.yml` 的 `--only=docs,facts` 补上 `--ci`——少了它这一步会退化成
   「修复完静默放过漂移」，即一个只亮绿灯的检查项。

**新增回归测试** `scripts/gate.d/_shared.test.mjs`（9 例，接进门禁 `docs` 阶段）：
真实临时仓库 + 真实 `git`，覆盖「不改写 ⇒ 不暂存」「已脏 + 被改写 ⇒ 暂存」
「已暂存 + 被改写 ⇒ 暂存」「就脏但没被碰 ⇒ 不暂存」「幂等」「命令失败 ⇒ 报红」
「CI 有差异 ⇒ 报红且不动索引」「CI 无差异 ⇒ 不误报」，外加一条断言
**`ci.yml` 里含 facts 阶段的 gate 调用必须传 `--ci`**（注入违规验证过会变红）。
`autoWork` 的失效方式恰是「看起来在修、其实没把修复带进提交」——本地跑一次门禁
完全看不出来，与 `30-docs.mjs` 开头那句「一个只会亮绿灯的守卫等于没有守卫」同源。

**顺带**：`dirtyPaths` 改用 `git status --porcelain -z`。默认输出会把非 ASCII 路径
转义成 `"\346\226\207.md"`（`core.quotepath=true` 是默认值），那样的串既读不了文件、
也不能直接喂给 `git add`。本仓库当前 0 个非 ASCII 路径，但这是个会**静默降级为
暂存失败**的坑，`-z` 下路径原样给出，一并消掉。
...
## 2026-09-22: 提交脚本的清理步骤能盖掉提交结果 —— 不可逆动作之后不允许有会抛的收尾

**问题**：`scripts/commit.mjs` 第 3 步是

```js
const commit = sh('git', ['commit', '-F', msgPath])
fs.rmSync(msgPath, { force: true })          // ← 裸删除，可能抛
if (commit.status !== 0) die(...)            // ← 结果判定在它后面
```

`fs.rmSync` 在本机（沙箱注入了 safe-delete 垫片，删除数超阈值即拒删并抛）**真的抛了**：
脚本带着栈回溯退出，而**提交其实已经建好**——栈回溯里没有任何一行说"提交成没成"，
用户看到的是一个失败的样子，事实是成功。这与本仓库 2026-09-22 记下的那条教训同源
（非幂等脚本的结论一律以最终状态为准），但这里是**脚本自己制造了歧义**，不是环境骗人。

**改动**：

- 抽出 `cleanupMsg()`：**永不抛**，且**以文件真的没了为准**——删完回查一次
  `existsSync`（垫片也可能「成功返回但文件仍在」或异步恢复），不一致就打印警告。
- **顺序对调**：先判 `commit.status` 再清理。不可逆动作之后的任何一步都不该能改写
  "它到底成没成"这个结论。
- 三处调用点（自校验失败 / `--dry-run` / 提交后）统一走 `cleanupMsg`。

**验证**：`--dry-run` 冒烟正常；本条目自身经 `commit.mjs` 提交（即端到端验证）。
...
而 `status` 取自 `git status --porcelain`（**工作区**）。但脚本最后一步是
`git commit -F <msg>`（**不带 `-a`**），提交的是**索引**。两者不一致时——工作区有改动、
却一处都没 `git add`——判据放行，于是：跑完几分钟门禁 → 生成提交消息 → 被 git 用
「no changes added to commit」拒掉。**真实原因在第一步之前就已知**（脚本自己在第 2 步
还打印了「暂存 0 个文件」），却被拖到最后一步、用一句指不到原因的报错表达出来。

**改动**：把索引检查提前到前置阶段。非 `--dry-run` 时索引为空即失败，并说明
「本脚本不替你暂存，只提交索引里的内容」；`--dry-run` 豁免（预览消息本就无需暂存）。

**验证**：索引为空 + 工作区脏 → 立刻失败、不跑门禁；`--dry-run` 正常预览。
...
而摘单一律取**正文开头**。于是流式期间那一行**一个字都不会变**——用户既看不出「还在长」
还是「卡住了」，也永远追不上早已流到几十字之后的正文，观感就是"折叠起来一直是开头那句"。

**改动**（`tauri/src`，纯呈现层）：

1. **摘要取哪一端由状态决定**（`registry/messageTypes.ts::messagePreviewFollowsLiveEdge`）：
   流式中取**末端**（这一行是走马灯，最新的内容才是要看的），定稿后取开头（摘要）。
   判据只取**协议字段** `status`，不嗅探内容形态——流式中的 JSON 尚未闭合，按内容判
   「是不是 JSON」会在某一片恰好闭合时突然换端，比不换更乱。
2. **文本与裁剪方向一起下发**（`useMessageContent.ts::MessageSummaryPreview`）：
   `messageSummaryPreviewOf` 返回 `{ text, liveEdge }`，截断的省略号落在**被截掉的那一端**
   （流式前置 `…`，定稿后置 `…`）。两件事**必须绑定返回**：只改文本不改裁剪方向，
   可见区里剩下的是**中间**那一段，最新内容照样被右端省略号吃掉。
3. **渲染器据此在正确的端裁剪**（`NodeShell.vue::.node-preview.live`）：流式态用
   `justify-content: flex-end` + 不可收缩子项把溢出留在**左**侧，由 `overflow: hidden`
   裁掉——`text-align: right` 对 `nowrap` 溢出无效，`direction: rtl` 会按 bidi 规则
   重排引号 / 括号 / 路径里的标点。`margin-right: auto` 负责"短内容不跑偏"：正剩余空间
   先被自动外边距吸收，文字仍贴左缘，只有真的溢出时才轮到 `flex-end` 生效。

**验证**：

- 渲染层复现（真实 store → 树 → DOM）：思考（始终单行）与运行中的工具行，流式增量后
  摘要里出现的是**新到的那一段**且带 `live` 类；定稿帧一到，摘要回到开头且 `live` 消失。
  两条用例已实测**修复前失败**（临时把 `liveEdge` 固定为 `false` 跑了一遍）。
- 纯函数四向：末端（前置省略号）/ 开头（后置省略号）/ 短内容（两端同串）/
  空内容（`text` 为空、`liveEdge` 原样透传），长度均为上限 + 1。
- **横向裁剪方向用真实引擎验过**：把 `.node-head` / `.node-preview` 那段 CSS 原样搬进
  独立 HTML，用 headless Edge（Chromium）截图——定稿行显示 `HEAD-MARK-xxxx…`（开头可见、
  右端省略），流式行显示 `xxxx…-TAIL-MARK`（末尾可见、左端被裁），短内容两态均紧贴标题
  无空档。这条断言 CSS 做不到（无布局），故以引擎截图为准。
...
而会话照常往下走。本地会话 `3ea6e4d0` 实测到 4 个这样的节点。

**根因是一处不对称**：`process_tool_calls_async` 的批尾收口只给**父节点**补终态
（`not_executed_patch`：`Completed` + `meta.failure_kind = "not_executed"`），**不写结果子节点**。
于是「有调用必有结果」这条不变量只在**请求视图**里成立（`flatten_chat_messages` 会为无结果的
ToolCall 合成占位 tool 结果，否则 provider 直接 400）——**模型看得见、用户看不见**，
所以它能长期潜伏而不被任何测试拦住。触发场景：交互模式下前一个工具失败 / 待审批 ⇒
本批剩余被 `break` 跳过（实测现场正是「前一个 `vdfs_read` 报错 → 后一个 `vdfs_read` 被跳过」）。

**修法**（两层，缺一不可）：

1. **机制**：新增 `not_executed_result`——未执行 / 中止的调用同样写一条 `role=tool` 的结果
   子节点（`Completed`，说明「没跑」并区分成因），与父节点补丁**成对**下发、一并落库、
   一并进下一轮上下文。`node-state-streaming.md` 补为不变量 #25。
2. **历史数据的显示兜底**：`registry/messageTypes::missingResultNoteOf` 按父节点自述的
   `meta.failure_kind` 给出一行交代（`not_executed` / `aborted`），`ToolCallNode` 在没有结果
   子节点时渲染它——修复前落下的数据也能立刻看懂，而不是留白。

回归：`tool_executor.test.rs` 两条（中止批 / 交互模式跳过：结果子节点与终态补丁都必须在），
前端两条（文案取值 + 「无标记就留白，不替工具编结果」）。

## 2026-09-20: 会话流体验三修 —— 等不到提示 / 重试跑到顶部 / 顺序偶尔错乱

VDFS 实时化之后暴露的三处体验回退，根因分散在**三层**，不是同一个 bug：

1. **「正在思考…」骨架消失** —— 它原先只有一个触发点：`TurnGroupNode`（Turn 节点已到、
   尚无子节点）。而 Turn 节点也是一条 VDFS 变更，而变更**不重放**：`created` 晚到或丢一次，
   用户点完发送就是一片空白。现在补第二个来源（`stores/sessionLive.needsTypingRow`）：
   会话节点说「在跑」而流里没有任何**可渲染**的在途节点 ⇒ 在流尾补同一条骨架（与
   Turn 骨架共用 `TurnPending.vue`；两个来源天然互斥，不会同时出现两条）。判据刻意排除
   「空壳节点」（`text` / `reasoning` 且正文为空）：它也是 `streaming` 却什么都画不出来，
   把它算作「已有内容」恰好会在最需要骨架的那一刻判成「不用补」。该判据与树构建共用
   `sessionTranscript.isBlankContentNode`（原先两处各写一份）。
2. **重试跑到页面顶部** —— 会话级错误原先是**流外**的横幅，而「这一轮没跑完」属于会话流的
   时间轴。现在它挂在**流尾**（与流内错误共用 `MessageErrorBox`；新增 `dismissible`，
   给会话级这一处独有的关闭入口），出现时同样滚进视野。有 Failed Turn 时依旧不显示——
   那时错误由节点承载，同一条错误不报两遍。
3. **顺序偶尔错乱（刷新才恢复）** —— 存储是 `seq` 的唯一分配者，前端为未落库的节点发本地号，
   靠**落库回包**换号。而用户自己发的那条**从来没有变更出口**（直连 `append_messages`），
   于是它永远只有本地号：本地游标一旦与存储水位错开（截断 / 重试 / 压缩重写之后很常见），
   同一棵树上就并存两套号，排序随之错位。修法：落库后从存储**读回权威版本**发一次
   `updated`（`SessionPlugin::emit_persisted_message`——发入参那条等于把「没有号」写进前端）；
   前端收到「本地号 ≠ 存储号」即记为分叉，**不在流式期间回读**（合并语义会拿存储里的半截
   正文覆盖本地更长的副本），转空闲时整份回读收敛（`reconcileTranscript`，与「终态补丁丢失」
   共用同一条自愈通路）。后端补一条单测钉住 `append_messages` 的「补号 + 严格递增」。

## 2026-09-20: 旧会话事件频道（`kind = "session"`）整体废除 —— 会话实时只剩 VDFS 一条通道

ADR-015 把会话显示搬到节点状态后，前端早已不看事件，但**进程内**还有两个消费者接着：
子智能体转播（`agent/host/subagent.rs`）与 CLI（`cli/src/client.rs`）。它们靠 `Status{busy|idle}`
判结束、靠 `Update` 补丁渲染正文——同一份「在跑什么」于是有两处真相，而 VDFS 那条已经是
权威的（且是全量节点视图，丢一帧不会卡住）。本次把剩下那一截收完：

1. **两个消费者改订阅 VDFS 变更**：`event_bus/subscribe`（收件）+ `vdfs/watch <会话地址>`
   （**开闸**）——只做前者是一条永远不响的频道（后端只向登记过路径的订阅者投递）。
   结束判据从「等 `Status idle` 帧」改为「读会话节点的 `status`」：`status != working`
   即本轮结束，结局从 `attributes.outcome` / `.error` 读。同一个判据在前端、子智能体、
   CLI 三处首次真正同源。
2. **「全量 vs 增量」的折算只能有一处**：`created` / `updated` 的载荷是**全量**，而消息补丁
   是**增量**（把全量当增量拼会叠字）。新增 `symbio_core::schemas::session::transcript`
   收下「节点 → 消息」逆投影与 `TranscriptPatchBuilder`——core 是进程内消费者的唯一共同层。
   前端 `services/vdfsTranscriptSync.ts` 是同一语义的另一实现，两处口径分叉的代价是
   CLI 与 UI 显示不同的正文（+8 例单测钉住折算方向）。
3. **后端删掉旧频道**：`orchestrator::broadcast` 不再发 `kind = "session"`，`session/workdir`
   那条与 VDFS 重复的 `{type:"data"}` 一并删除，`KIND_SESSION` 常量移除——`event_bus`
   词表只剩 `system` / `vdfs`，退化为**纯传输层**（不再认识任何业务频道语义）。
   子智能体与 CLI 遇到缺节点视图的变更会**留痕**（`⚠ 变更缺节点视图，已跳过`），
   而不是像从前那样静默丢一帧——静默跳过正是最难查的那类事故。

## 2026-09-20: 覆盖率阈值太低也会吭声了；沙箱不再让覆盖率整步跳过

1. **覆盖率棘轮补上「太低」那半边的提示**。阈值只在**低于**它时才红，太高则完全静默
   ⇒ 它像 `BASELINE.vitestTests`（400 → 661）那样悄悄落后。实测行覆盖率 **62.53%**、
   阈值 40，**删掉两成覆盖也不会有人知道**。现在超出 10 个点就打黄字提醒上调。
2. **沙箱不再让这一步整步跳过**。沙箱拦的是 vitest 开跑前清理 `coverage/` 那一步
   （`V8CoverageProvider.clean`），**不是测试本身**。改为以 `--coverage.clean=false`
   重跑，而不是直接放弃判定——"没检查"不该被伪装成"跳过"。
3. `grep-audit.mjs` 头部的「对应 PLAN §S-4」指向一份**已不存在**的文档（`docs/archive/`
   下三份计划里都没有 `S-*` 编号）。规则编号由此继承自找不回来的清单，故
   **S-001 / S-003..S-006 是跳号，不是被删的规则**——写清楚，免得有人去"补号"。

## 2026-09-20: 门禁汇总不再把「没判定」画成绿勾

本机沙箱会拦批量删除，`vitest --coverage` 与 `vite build` 因此跑不起来。门禁的处理是
「记通过 + 一行小字说明」，于是汇总里出现 `✓ vitest run --coverage — 沙箱拦截 ⇒ 本步未判定`
——**一颗绿勾配一行小字，读起来仍然像"过了"**。这正是本轮反复在找的那类假信号。

`record()` 现在接受 `'skipped'`：汇总打 `⊘`、计入「另有 N 项未判定」、不计入通过数。
**（没判定 ≠ 通过。）**
...
**794**，前端半边（`vitestFiles` / `vitestTests`）自「二次复核」定到 **31 / 400** 之后
再没动过——而实测已是 **47 / 661**。也就是说**删掉 261 个用例（占现有四成）也不会红**，
正是棘轮本该拦住的事。

**病根是"没有提示"**：Rust 半边超过基线会打黄字「请更新 `BASELINE.rustTests`」，前端
半边只 `dim` 打印一行「（基线 31/400）」——**没有提示的棘轮等于没有棘轮**，于是它涨了
261 个用例都没人被告知。

### 修法

- `vitestFiles: 31 → 47`、`vitestTests: 400 → 661`；
- 前端半边补上与 Rust 同款的「超过基线 ⇒ 黄字提示 + 记录标注『基线待更新』」；
- 新分支**已实际执行验证**（临时把基线压到 660 跑一次，确认黄字与标注都出现，再改回）——
  本轮的教训正是"没被执行过的分支会烂"。

### 为什么这一半可以直接钉实测值，而覆盖率阈值不能

**用例数与平台无关**：全仓 `*.spec.ts` 零 `skipIf` / `runIf` / `process.platform` 分支，
仅有的两处 `it.each`（`messageRenderers.spec.ts`）遍历的是静态常量数组
（`ALL_RENDERERS` / `MESSAGE_TYPES`）⇒ 用例数由源码唯一决定，Linux runner 与本地必然同数。

覆盖率则是**分支命中率**，路径处理等平台分支会真的不同 ⇒ `tauri/vitest.config.ts` 的
阈值（40 / 38 / 38）**仍留待决策**，须先在 CI 取实测值。两者病根相同（棘轮没人拧），
已在配置注释里互相交叉引用。

### 验证

- `gate.mjs --only=frontend`：**4/4**，输出 `47 文件 / 661 用例（基线 47/661）`
- `gate.mjs --only=docs,facts`：**16/16**
- `node --check scripts/gate.mjs`：通过
