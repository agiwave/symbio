...
## 通用：实体管理链路（`{plugin}/entities/*`）

| # | 环节 | 代码位置 | 说明 |
|---|------|---------|------|
| 1 | 公共流程 | `symbio_core/entities.rs` | `EntityProvider` trait + `dispatch`（list/get/upload/delete/status） |
| 2 | 驱动方 | `session`（`worker/session`）、`model`（`worker/model`）、`agent`、`skill`、`mcp`、`setting` | 各自实现 `EntityProvider`；注册表见 `symbio_core/entities.rs` 的 `provider_registry()` |
| 3 | 机制详解 | [design/entity-management-mechanism.md](../design/entity-management-mechanism.md) | 能力开关（`EntityCapabilities`）等 |
