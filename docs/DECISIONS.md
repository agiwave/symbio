...
评审 A6（[archive/plugin-system-review-2026-09.md](./archive/plugin-system-review-2026-09.md)）
记录了一处**双源**：`Plugin::meta()` 返回 `PluginMeta`（含 `id`/`name`/`version`/
`author`），而 trait 注释**自承**「唯一运行期消费方是 `composite/vdfs.rs`」
「插件的身份实际来自各自的 `PLUGIN.yml`」。同一事实两个来源，其中一个是
「刻意的但几乎没人用」。

后果已经显形：`PluginEntry` 的 `title` / `description` / `version` 取自 `meta()`，
而**停用的插件刻意不被构造**（见 `KEY_ENABLED`——它不该启动任何后台行为），
于是那几个字段在停用态为空，消费方按目录名兜底。同一个「插件叫什么」在不同地方
可能给出不同答案：`PLUGIN.yml` 一个、`meta()` 一个、目录名一个。

从「一个插件 = 一个目录、目录可整体搬走」这条不变量（第三方插件规范 I1）看，
**身份是目录的属性，不是构造物的属性**——停用与否不改变「它是谁」。

**决策**

1. **身份归 manifest**。`PLUGIN.yml` 新增身份保留键，运行期**唯一**消费路径是 manifest：

   | 键 | 内容 | 对应 `PluginMeta` 字段 |
   |---|---|---|
   | `plugin_title` | 展示标题 / 挂载点标题 | `name` |
   | `plugin_description` | 语义描述 | `description` |
   | `plugin_version` | 版本 | `version` |
   | `plugin_author` | 作者 | `author` |

2. **`PluginMeta` 语义从「元信息（身份声明）」改为「出厂自述」**，字段分两类：

   | 类 | 字段 | 何时读 |
   |---|---|---|
   | **出厂种子** | `name` / `description` / `version` / `author` | **仅装配期一次**：容器构造出插件后把它投影进 manifest（键缺失才写） |
   | **挂载点呈现** | `order` / `icon` / `hidden` / `root_access` | 运行期（合成挂载点目录节点时） |

3. **`plugin_version` 从「纯投影键」升级为「真身份键」**。它原先只用于把
   `PluginEntry::version` 注入插件管理插件的表单模型（`DetailAction.when` 的判据），
   值绕了一圈（manifest 无版本 → `meta()` → `PluginEntry` → 注入）。现在 manifest
   是真值，投影与身份**同源**——键名不变，语义收敛。`plugin_required` /
   `plugin_can_disable` 仍是**纯投影键**（没有 manifest 对应物），两者不混。

4. **保留键集合收口为 `RESERVED_KEYS`**，`PluginDir` 的读写统一遍历它（剥离 /
   保留各一处实现），不再逐个 `map.remove`——「哪些键属于装配方」从此只有一份清单。

5. **为 A5 预留两个键**：`plugin_api`（要求的宿主插件 API 版本）与 `plugin_grants`
   （宿主授予的能力，闭集见第三方插件规范 §7）。本期只**定义与解析**，不强制——
   「授权校验」是第三期的事。

**理由**

- **身份跟着目录走**：搬走目录 = 带走身份（连同一个改过的标题、一个本地化的描述）。
  若身份只在代码里，同一个插件在所有智能体里只能叫同一个名字。
- **停用不等于不存在**。「这个插件叫什么」与「它开没开」是两件事：把前者绑在
  构造物上，就等于「停用后连名字都没了」——而插件管理插件的列表里**必须**能显示
  一个停用插件的名字（否则用户只看到一个目录名）。
- **装配期一次性投影不是「双源」**：出厂声明 → 落盘值是**单向**的，落盘后代码里的
  种子不再被读（除首次落位）。这与「`Default` 与用户配置」是同一种关系，
  而不是「同一事实两处维护」。
- **挂载点呈现不投影**（`order` / `icon` / `hidden` / `root_access`）：它们是**挂载点**
  的属性，不是插件的属性——停用即无挂载点，`order` 随之无意义。这与「身份必须常在」
  正好相反，因此两类字段走两条路（这正是本 ADR 把 `PluginMeta` 重新分类的依据）。

**后果**

- `PluginEntry` 的 `title` / `description` / `version` 改从 manifest 读 ⇒ **停用插件
  也有身份**；`registry.rs` 注释里「那几个字段可能为空，消费方按目录名兜底」的
  免责说明随之失效（兜底只留给**从未落位过**的目录，即新装但从未构造成功的）。
- `composite/vdfs.rs::dir_node` 的 `title` / `description` 改从 manifest 读；
  `root_access` / `hidden` 仍取 `meta()`。
- **本期不删 `PluginMeta` 的身份字段**。删除它们需要「**不构造也能拿到出厂身份**」的
  能力——即一张按工厂 id 索引的静态自述注册表。那是第二期 A1（provider 两级解析）
  的天然产物；本期先让**运行期消费路径**单源化（这是 A6 的实质），出厂声明的载体
  留待第二期一并迁移。**未完成的部分明确记录在此，不假装已解决。**
- 装配期写盘：`PluginDir::seed_identity` 只在键缺失时写，用户改过的值不被覆盖——
  与既有 `ensure_manifest`（只补身份字段、已存在则不动）同一语义；写入时
  `save` / `set_enabled` / `remove_keys` 一律**保留**保留键（同一处实现），
  否则「在设置页保存一次配置」会把身份冲掉。
- 内置 16 个插件的 `metadata()` **不改一行**：出厂种子仍声明在代码，只是它的
  消费方式从「运行期每次读」变成「装配期投影一次」。
- **一个已知边界（不粉饰）**：身份只在**首次装配**时落位，而装配只发生在插件被
  构造时。因此**遗留工作区里「一直停用、从未构造过」的插件**在新版本下仍然没有
  身份——要等用户启用一次才落位。这是**现状等价而非退化**（改前同样拿不到
  `meta()`，所以它此前也没有身份）。彻底解决需要上面那条静态注册表，与「删除
  `PluginMeta` 身份字段」是同一件事，一并留到第二期。
- 时序上不会出现「挂载中却无身份」：投影发生在 `mount_child` 构造成功之后、
  插件进入实例表**之前**，而一切读取（`entries()` / 挂载点节点）都发生在装配之后。
- 第三方插件规范 §9 第一期（manifest 成形）由此落地 A5 / A6；A4 的能力闭集见 §7。
...
评审 A2：`Plugin` trait 只有 `meta` / `route` / `traverse` / `get_vfs_provider` /
`vdfs_dispatch`，**没有生命周期钩子**。「停用」的语义因此只能是「**不构造**」
（ADR-032 沿用的一条边界）：插件没有任何机会做清理——关监听、停后台任务、落盘，
全都要靠 `Drop`。而 `Drop` 是同步的：不能 await、不能报错、也不区分
「停用」「卸载」「退出」。

对第三方插件（第二期）这条是硬阻塞：`ExternalPlugin::stop()` 要发 `shutdown` 帧 →
等 `ok` → 关 stdin → 等进程退出 → 超时则 kill。**没有钩子就没有地方放这段代码**。
...
2. **`start` 为什么是同步的**：装配路径是**同步**的——`HomePlugin::build` →
   `Composite::build` → `PluginRegistry::mount_all`，而工厂签名
   （`ObjectConstructor`）本身非 async，改成 async 要波及 inventory 注册表与全部
   工厂。**在那里 await 不可用**，所以 `start` 只能是同步的。
   需要异步初始化的插件（外部插件）走别的两步：**构造时同步 spawn 子进程** +
   **首次调用前惰性完成 `init`**，而不是把装配改 async。

3. **`stop` 为什么是异步的**：它的全部调用点（停用 / 卸载）都已经在 async 上下文
   （`VdfsProvider::dispatch` 的动词），且清理本身需要 await（发帧、等进程退出、
   flush 落盘）。同步的 `Drop` 做不了这些。
...
5. 接线（本期三个）：
   ...
   | 动作 | 现有实现 | 加钩子后 |
   |---|---|---|
   | 装配 | `mount_all()` 构造 | 构造成功后逐个 `start(ctx)`（身份投影之后、进实例表之前） |
   | 停用 | `set_enabled(name, false)` 摘实例 | 摘出实例表**之后立即** `stop(Disabled)` |
   | 卸载 | `uninstall(name)` 摘实例 + 删目录 | 摘出实例表 → `stop(Uninstalled)` → **再**删目录 |

   顺次的要点是「**stop 早于任何破坏性动作**」，不是「早于摘实例表」：先把插件摘出
   实例表（它立刻不再被路由命中）、再给它时间清理——清理**不该**阻塞「停用」本身
   （用户按下停用就该立即生效）。

6. **`start` 失败怎么处置**：记 `plugin_error!` 并**保留挂载**（不摘掉）。
   这与 `provider_of` 已有的口径一致——「配了一半的插件也要出现在表里，否则用户
   既看不到它、也删不掉它」。启动时炸掉的插件同样是**要被看见**的：摘掉它，用户
   只看到「插件不见了」；留着它，每次调用会给出明确的错误。

7. **`stop` 失败怎么处置**：记 `plugin_warn!` 并**继续**（停用照常生效、卸载照常删
   目录）。与 `start` 相反——这里的用户意图是「**让它停下**」：用清理失败去否决它，
   等于给插件一个「我停不掉」的否决权，而用户此刻已经没有别的手段了。失败是可观测的
   （日志），但**不该反向卡住**用户的动作。

**理由**

- **钩子必须可选**：16 个内置插件不该因为加钩子而改一行；「加机制不改变现状」是
  本批（ADR-032 起）的一贯口径，也是 `ExternalPlugin` 能与内置插件共存的前提。
- **`stop` 早于任何破坏性动作**：摘出实例表只是让它不再被路由命中，插件的数据
  此刻还在；真正的截止线是「**删目录**」。顺序反了就等于让插件在数据已经没了之后
  再去做清理。卸载时尤其致命——`stop(Uninstalled)` 是插件**最后**一次能写盘的机会。
- 不把 `start` 做成 async 也避免了「装配期需要 runtime」：装配发生在
  `HomePlugin::build`（同步、可能还没有 tokio runtime 上下文）里。

**后果**

- `PluginRegistry::set_enabled` / `uninstall` 变 **async** ⇒ 调用方
  （`composite/vdfs.rs` 的两个注册表动词）随之 await；涉及它们的测试改为 await。
- 内置插件行为与今天完全一致（默认空实现）。
- **未完成（明确记录，不假装已做）**：「进程退出」与「`home/reload` 丢弃旧容器」
