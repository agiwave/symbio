...
- `chat/send` 退化为「写 `inbox` + 等待本轮」的**兼容包装**，顶层调用方的现有体验不变，语义 owner 收敛到 `inbox`。
- 活动会话状态上新增队列，并由它统一收敛「是否正在跑」，取代此前由单次调用隐式携带的忙碌判断。
- 取消一条**已排队**消息 = 删除对应 `inbox` 条目；**正在处理**（已出队）的那条不适配删除语义，取消仍走既有中止路径。
- 本条记录的是**已确定的机制取向**；代码落地与 e2e 覆盖为后续步骤，不在本条内宣称已实现。
