...
export default defineCase('T11 压缩节点协议：compression 节点的开始/终态与重写收敛', async () => {
  ...
  try {
    ...
    const doneTurns = () =>
      ops.filter((o) => o.op === 'upsert' && o.message?.type === 'turn' && TERMINAL.has(o.message?.status)).length;
    ...
    await waitFor(
      () => ops.some((o) => o.op === 'upsert' && o.message?.type === 'compression'),
    ...
    const timeline = ops.map(
      (o) =>
        `#${o.seq} ${o.op}${o.message ? ` [${o.message.type ?? '-'}/${o.message.role ?? '-'}/${o.message.status ?? '-'}] ${String(o.message.id ?? '').slice(0, 8)}` : ''}${o.message_id ? ` ->${String(o.message_id).slice(0, 8)}` : ''}`,
    );
    ...
    const compFrames = ops.filter((o) => o.op === 'upsert' && o.message?.type === 'compression');
    ...
    for (const [id, frames] of episodes) {
      ...
      assertEq(frames[0].status, 'streaming', `compression ${String(id).slice(0, 8)} 首帧应为 streaming`);
      assert(
        String(frames[0].content ?? '').length > 0,
        `compression ${String(id).slice(0, 8)} 开始帧应带非空正文（否则前端是一个没有内容的空壳节点）`,
      );
      // ④ 终态帧：completed 或 failed，且不再是非终态
      ...
      assert(
        ...
        `compression ${String(id).slice(0, 8)} 终态应带结果说明（成功写"压掉了多少"，失败写原因）`,
      );
    ...
    for (const [id, frames] of episodes) {
      ...
      assert(
        frames.every((f) => f.parent_id == null),
        `compression ${String(id).slice(0, 8)} 应是根级节点（不是某个 Turn 的子节点）`,
      );
      const firstFrame = ops.find((o) => o.op === 'upsert' && o.message?.id === id);
      assert(firstFrame.message.parent_id == null, `compression ${String(id).slice(0, 8)} 不得挂在 Turn 下`);
      // 它之后的第一个 Turn 必须是**新建**的（该 id 在压缩开始之前没出现过）
      const nextTurn = ops.find(
        (o) => o.op === 'upsert' && o.message?.type === 'turn' && o.seq > (frames[frames.length - 1].seq ?? 0),
      );
      assert(nextTurn, `compression ${String(id).slice(0, 8)} 之后应有一个 Turn`);
      const seenBefore = ops.some(
        (o) => o.op === 'upsert' && o.message?.id === nextTurn.message.id && o.seq < beginSeq,
      );
      assert(
        !seenBefore,
    ...
    // ⑥ 重写收敛：压缩成功后流上应有 remove（被压掉的历史）与快照 upsert
    const snapshotFrames = ops.filter((o) => o.op === 'upsert' && o.message?.meta?.compacted === true);
    ...
    assert(removes.length > 0, '压缩重写应向流下发被压掉消息的 remove（前端据此就地收敛）');
    // remove 的目标必须是本连接上出现过的节点（删除未知 id 是协议违例，
    // 前端无从收敛——它既不知道该删谁，也不知道自己少了什么）
    for (const rm of removes) {
      const appeared = ops.some(
        (o) => o.op === 'upsert' && o.message?.id === rm.message_id && o.seq < rm.seq,
      );
      assert(
        appeared,
        `remove 了流上从未出现过的节点 ${String(rm.message_id).slice(0, 8)}\n时间线:\n${timeline.join('\n')}`,
      );
    }
