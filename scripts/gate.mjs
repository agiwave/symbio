...
const BASELINE = {
  ...
  // 797 → 805：S22——旧会话事件频道（`kind = "session"`）整体废除后的**共享折算层**
  //   （`symbio_core/schemas/session/transcript.rs`，+8）：全量 `created` 出整段正文 /
  //   `appended` 只出 delta 并累积（**把全量当增量拼会叠字**，这例就是防线）/
  //   `updated` 正文未变时补丁不带 content（纯状态迁移）/ 正文增长时只出新尾巴 /
  //   组合节点（ToolCall）的 JSON 视图能读回原消息 / 缺节点视图**显式上报**而非静默跳过 /
  //   `deleted` 清掉合并视图 / 会话节点不折算成消息补丁（运行态不混进转写）。
  //   钉的是折算的**唯一实现**：前端 `services/vdfsTranscriptSync.ts` 是同一语义的
  //   另一实现，两处口径分叉的代价是 CLI 与 UI 显示不同的正文。
  // 805 → 806：S23——「顺序偶尔错乱」的根因之一：存储是 `seq` 的唯一分配者，
  //   前端为未落库的节点发本地号、靠**落库回包**换号，而用户自己发的那条从来没有
  //   变更出口。新增 `chat_session.test.rs` 钉住 `append_messages` 的「补号 +
  //   跨次严格递增」（Lamport 计数器），+1。缺它时存储不再补号不会有任何测试失败：
  //   前端只是「偶尔顺序错乱、刷新才恢复」。
  ...
  // 661 → 672：S23——会话流体验三修 + 工具响应兜底（+11）。骨架兜底判据六例
  //   （`sessionLive.spec.ts`：不在跑不补 / 在跑且空手才补 / 有在途正文不补 /
  //   **空壳节点不得抑制骨架** / 工具卡片算内容 / 空壳与真内容并存）+
  //   存储号与本地号分叉后的对账三例（`sessions.spec.ts`：分叉 ⇒ 不在途中回读、
